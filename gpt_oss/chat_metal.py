"""
Harmony chat with tools - Metal backend version
"""
...
# Only import torch if not using metal backend
try:
    import torch
except ImportError:
    torch = None
...
# Try to import Docker-based Python tool, fall back to local if Docker unavailable
try:
    from gpt_oss.tools.python_docker.docker_tool import PythonTool
    PYTHON_TOOL_AVAILABLE = "docker"
except ImportError:
    PYTHON_TOOL_AVAILABLE = None


# Import stateful Python tool
try:
    from gpt_oss.tools.python_stateful import StatefulPythonTool
    STATEFUL_PYTHON_AVAILABLE = True
except ImportError:
    STATEFUL_PYTHON_AVAILABLE = False
...
class MetalTokenGenerator:
    """Wrapper for Metal backend to match TokenGenerator interface"""
    
    def __init__(self, checkpoint: str, context: int = 8192, device=None):
        """Initialize Metal model and context
        
        Args:
            checkpoint: Path to the Metal model file
            context: Maximum context length
            device: Ignored for Metal backend (uses Metal/GPU automatically)
        """
    ...
    def generate(self,
        ...
        """Generate tokens matching the standard TokenGenerator interface
        ...
        Args:
            ...
            return_logprobs: If True, yield (token, logprob) tuples (not supported)
        
        Yields:
            Token IDs one at a time
        """
        if return_logprobs:
            raise NotImplementedError("Metal backend doesn't support returning logprobs")
            
        stop_tokens = stop_tokens or []
        
        # Create fresh context for each generation
        self.context = Context(self.model, context_length=self.context_length)
        
        # Append all prompt tokens to context
...
def get_user_input():
    # For metal backend or when torch is not available, just use simple input
    if torch is None or not hasattr(torch, 'distributed'):
        return input()
    
    # For distributed torch backends
...
def main(args):
    match args.backend:
        case "metal":
            generator = MetalTokenGenerator(args.checkpoint, args.context)
        case "triton":
    ...
    if args.python:
        # Default: Use stateful local Python executor
        python_tool = None
        
        if not args.python_stateless:
            # Default to stateful local executor
            try:
                python_tool = StatefulPythonTool()
                print(termcolor.colored("Python:", "cyan"), "Stateful executor (maintains state, REPL-like)", flush=True)
            except Exception as e:
                print(termcolor.colored("Warning:", "yellow"), f"Failed to initialize stateful executor: {e}", flush=True)
        
        # Fallback to stateless if requested or if stateful failed
        if python_tool is None:
            if args.python_stateless:
                print(termcolor.colored("Note:", "yellow"), "Using stateless execution (--python-stateless flag)", flush=True)
            else:
                print(termcolor.colored("Note:", "yellow"), "Falling back to stateless execution", flush=True)
            
            # Use Docker stateless executor
            try:
                python_tool = PythonTool()
                print(termcolor.colored("Python:", "cyan"), "Stateless Docker executor (isolated)", flush=True)
            except Exception as e:
                print(termcolor.colored("Error:", "red"), f"Docker not available for stateless mode: {e}", flush=True)
                print(termcolor.colored("Error:", "red"), "Stateless mode requires Docker. Use default mode (without --python-stateless) for local execution.", flush=True)
                raise
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Chat example with Metal backend support",
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        type=str,
        help="Path to the model checkpoint (SafeTensors for torch/triton/vllm, Metal format for metal backend)",
    )
    parser.add_argument(
        "-r",
        "--reasoning-effort",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use stateless Python executor (disables default stateful behavior)",
    )
    parser.add_argument(
        "--developer-message",
        default="",
    ...
    if int(os.environ.get("WORLD_SIZE", 1)) == 1:
        histfile = os.path.join(os.path.expanduser("~"), ".chat_metal")
        try:
